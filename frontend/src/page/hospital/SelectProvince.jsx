...
const SelectProvince = () => {
  ...
  const getImageUrl = (provinceName) => {
    const provinceData = provincesData.find((data) => data.name.toLowerCase() === provinceName.toLowerCase());
    return provinceData ? `${process.env.PUBLIC_URL}/${provinceData.imageUrl}` : `${process.env.PUBLIC_URL}/images/default.png`;
  };
          ...
          <div className="province-cards">
            {filteredProvinces.length > 0 ? (
              filteredProvinces.map((province) => (
                ...
                >
                  <img src={getImageUrl(province.name)} alt={province.name} className="province-image" />
                  <h3>{province.name}</h3>
                </div>
              ))
            ) : (  
